locals {
  ...
  # Computed from path.root (envs/prod) so the lambda module never has to guess its depth.
...
module "dynamodb" {
  ...
  # Two layers: deletion protection stops the AWS-side delete, prevent_destroy stops Terraform planning one.
...
module "lambda" {
  ...
  # Caps blast radius and stops this function draining the account concurrency pool.
...
module "api" {
  ...
  # Explicit throttle rather than whatever the account default happens to be.
